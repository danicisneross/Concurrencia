...
int increment(void* p){     //p es un tipo de puntero generico que puede apuntar a cualquier tipo de objeto
    ...
    for(int i = 0; i < *d->iterations; i++) {
        ...
        mtx_lock(&d->mutex[pos]);           //utilizamos un mutex para cada posicion y asi nos aseguramos de que solo un hilo puede acceder a dicha posicion simultáneamente   
...
int swap(void *p){
    struct thread_data* d = (struct thread_data *) p;       //inicializacion de d y conversion del puntero p al struct thread_data
...
int main (int argc, char **argv){
    ...
    struct thread_data* thread_data;        //argumentos del hilo usado para el incremento
    struct thread_data* thread_data_swap;   //argumentos del hilo usado para el intercambio
    thrd_t* thr_swap;                       //array de hilos intercambio
    thrd_t* thr;                            //array de hilos de incremento
    mtx_t* mutex;                           //array de mutex
