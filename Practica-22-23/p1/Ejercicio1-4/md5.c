...
int queue_in_out (void* p){
    ...
    end(args->out_q); //se terminó de llenar out_q
    return 0;
}
...
void sum(struct options opt) {
    ...
    end(in_q); //aquí ya hemos terminado de insertar

    inout_args.in_q = in_q;
    inout_args.out_q = out_q;
    thrd_create(&inout_thrd, queue_in_out, &inout_args);
