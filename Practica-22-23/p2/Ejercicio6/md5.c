...
void read_hash_file(char *file, char *dir, queue q) {
    ...
    while(fgets(line, MAX_LINE_LENGTH, fp) != NULL) { //se lee el archivo de hash
        char *field_break;
...
int read_files2(void *p){
	struct read_file_struct* args = p;
		
    read_hash_file(args->file, args->dir, args->in_q);
...
//ejer6
int hashcheck(void* p){
    struct file_md5 *md5_in, md5_file;
    struct hashcheck* args = p;
...
void check(struct options opt) {
    int i;
    queue in_q;
    struct read_file_struct read_hash_file_args;
    thrd_t read_hash_file_thrd;
    
    thrd_t* hashcheck_thrd = malloc(sizeof(thrd_t) * opt.num_threads);
    struct hashcheck hashcheck_args;

    in_q  = q_create(opt.queue_size);

    read_hash_file_args.file = opt.file;
    read_hash_file_args.dir = opt.dir;
    read_hash_file_args.in_q = in_q;

    thrd_create(&read_hash_file_thrd, read_files2, &read_hash_file_args);

    //ej6
    hashcheck_args.in_q = in_q;
    
    for (i=0; i < opt.num_threads; i++){
        thrd_create(&hashcheck_thrd[i], hashcheck, &hashcheck_args);
    }

    thrd_join(read_hash_file_thrd, NULL);
    for (i=0; i < opt.num_threads; i++){
        thrd_join(hashcheck_thrd[i], NULL);
    }
